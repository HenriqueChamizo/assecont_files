    ...
    public class GPS
    {
        ...
        public GPS(BD bd, string fileName, int ObrigacaoClientePeriodo, int ArquivoIndice, bool Publish, out int GpsInd)
        {
            ...
            if (Path.GetExtension(fileName).ToLower() == ".txt")
                GpsInd = LerArquivo(fileName);
            else
                throw new Exception("O arquivo deve estar em formato .txt");
        }

        public int LerArquivo(string fileName)
        {
            #region Campos
            /*Cliente*/
                ...
                foreach (string linha in Arquivo)
                {
                    ...
                    if (contadorLinha == 9)
                    {
                        if (linha.Substring(Campos[valorINSS].Inicio, Campos[valorINSS].Tamanho).Trim() != "")
                            cliente.valorINSS = Convert.ToSingle(linha.Substring(Campos[valorINSS].Inicio, Campos[valorINSS].Tamanho));
                    }
                    ...
                    if (contadorLinha == 15)
                    {
                        cliente.valorEntidades = Convert.ToSingle(linha.Substring(Campos[valorEntidades].Inicio, Campos[valorEntidades].Tamanho));
                    }

                    if (contadorLinha == 21)
                    {
                        cliente.valorPrincipal = Convert.ToSingle(linha.Substring(Campos[valorPrincipal].Inicio, Campos[valorPrincipal].Tamanho));
                    }
        ...
        bool ValidaDomestica(int ObrigacaoClientePeriodo)
        {
        ...
        int ArquivoUpload(byte numero, string fileName)
        {
            ...
            return uploadArquivoNovo(ObrigacaoClientePeriodo, fileName, (Tipo.ObrigacaoArquivoTipo)arquivotipo, (Tipo.ObrigacaoArquivoSubTipo)arquivosubtipo, 
                false, user, memorando, numero);
        }

        public int uploadArquivoNovo(int ObrigacaoClientePeriodo, string Arquivo, Tipo.ObrigacaoArquivoTipo tipo, Tipo.ObrigacaoArquivoSubTipo subtipo, bool Provisorio, int User, string Memorando, byte ArquivoNumero)
        {
