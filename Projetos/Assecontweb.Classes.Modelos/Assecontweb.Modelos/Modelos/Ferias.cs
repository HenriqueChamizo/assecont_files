    ...
    public class Ferias
    {
        ...
        private int LerArquivotxt(string fileName, int ObrigacaoClientePeriodo /*, bool ValorAtualizavel */)
        {
            ...
            List<Campo> Campos = new List<Campo>()
            {
                ...
                new Campo() {Inicio = 24, Tamanho = 4}, // saldoDias
            ...
            //string linha in Arquivo)
            {
                ...
                if (contadorLinha == 0)
                {
                    if (linha.Substring(35, 18) != "CONTROLE DE FERIAS")
                    {
                ...
                if (contadorLinha == Arquivo.Count-2)
                {
            ...
            else
            {
                try
                {
                    foreach (Funcionario Func in cliente.listFuncionarios)
                    {
                        ...
                        cmd.Parameters.AddWithValue("@P_SALDO_DIAS", Func.saldoDias.ToString().Replace(',', '.'));
                        cmd.Parameters.AddWithValue("@P_TOTAL_FALTAS", Func.totalFaltas);
                        cmd.Parameters.AddWithValue("@P_POSICAO", Func.posicao);
                        cmd.Parameters.AddWithValue("@P_PERIODO", Func.per);
                        cmd.Parameters.AddWithValue("@P_PROPORC", Func.proporc);
                        cmd.Parameters.AddWithValue("@P_PERIODO_AQUISICAO1", Func.periodoAquisicao1);
                        cmd.Parameters.AddWithValue("@P_PERIODO_AQUISICAO2", Func.periodoAquisicao2);
                        cmd.Parameters.AddWithValue("@P_PERIODO_GOZO1", Func.periodoGozo1);
                        cmd.Parameters.AddWithValue("@P_PERIODO_GOZO2", Func.periodoGozo2);
