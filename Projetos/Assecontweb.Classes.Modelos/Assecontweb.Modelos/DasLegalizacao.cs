        ...
        public class DasLegalizacao
        {
                ...
                List<Campo> Campos = new List<Campo>()
            {
                ...
                foreach (string linha in Arquivo)
                {
                    ...
                    if (contadorLinha == 23)
                    {
                        cliente.valorPrincipal = Global.CultureInfoValue(linha.Substring(Campos[valorPrincipal].Inicio, linha.Length).Trim());
                    }
                    if (contadorLinha == 24)
                    {
                        cliente.multa = Global.CultureInfoValue(linha.Substring(Campos[multa].Inicio, linha.Length).Trim());
                    }
                    if (contadorLinha == 25)
                    {
                        cliente.jurosEncargos = Global.CultureInfoValue(linha.Substring(Campos[jurosEncargos].Inicio, linha.Length).Trim());
                    }
                    if (contadorLinha == 26)
                    {
                        cliente.valorTotal = Global.CultureInfoValue(linha.Substring(Campos[valorTotal].Inicio, linha.Length).Trim());
                    }
                    if (contadorLinha == 29)
                    {
            ...
            private int SalvarBanco(Cliente cliente, int ObrigacaoClientePeriodo)
            {
                ...
                else
                {
                    try
                    {
                        ...
                        cmd.Parameters.AddWithValue("@P_VALOR_PRINCIPAL", cliente.valorPrincipal);
                        cmd.Parameters.AddWithValue("@P_VALOR_JUROS_ENCARGOS", cliente.jurosEncargos);
                        cmd.Parameters.AddWithValue("@P_VALOR_MULTA", cliente.multa);
