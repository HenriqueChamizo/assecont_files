...
namespace Assecontweb
{
    namespace Modelos
    {
        public class DarfPago
        {
            ...
            void CapturaDarfsPagos(string tableContent)
            {
                ...
                for (int i = 0; i < trContents.Count; i++)
                {
                    ...
                    if (tipo.ToUpper() == "DARF")
                    {
                        ...
                        Darf.valorPago = Global.CultureInfoValue(Regex.Replace(TdContents[6], @"<[^>]*>", string.Empty).Trim().Replace("&nbsp;", ""));
            ...
            int getLinha(List<string> tdContents, string Campo)
            {
                int result = -1;

                for (int i = 0; i < tdContents.Count; i++)
                {
                    if (tdContents[i].Contains(Campo))
                    {
                        result = i;
                        break;
                    }
            ...
            private void CapturarComprovate(string tableContent)
            {
                ...
                int LinhaCompetencia = getLinha(tdContents, "Período de Apuração");
                int LinhaVencimento = getLinha(tdContents, "Data de Vencimento");
                int LinhaReceita = getLinha(tdContents, "Valor no Código de Receita");
                int LinhaDocumento = getLinha(tdContents, "Número do Documento");
                int LinhaCodigoReceita = getLinha(tdContents, "Valor no Código de Receita");
                int LinhaValorTotal = getLinha(tdContents, "Valor Total");
                ...
                for (int linha = 0; linha < tdContents.Count; linha++)
                {
                    ...
                    //if (linha == 17)
                    if (linha == LinhaDocumento + 1)
                        Darf.documento = Regex.Replace(tdContent, @"<[^>]*>", string.Empty).Trim();

                    //if (linha == 21)
                    if (linha == LinhaValorTotal + 1)
                        Darf.valorPago = Global.CultureInfoValue(Regex.Replace(tdContent, @"<[^>]*>", string.Empty).Trim());
                }
            ...
            private void GravarDarfsSQL(List<Comprovante> Comprovantes)
            {
                ...
                foreach (Comprovante Darf in Comprovantes)
                {
                    ...
                    string s = "SET DATEFORMAT DMY " +
                               "SELECT * FROM Darfs " +
                               "INNER JOIN ObrigacoesArquivos ON OBRARQ_IND = DARF_OBRIGACAOARQUIVO " +
                               ...
                               "DARF_CODIGORECEITA = {1} AND OBRCLIPER_CLIENTE = {2} AND DARF_DARFPAGO IS NULL AND " +
