(function ($) {
    $.fn.extend({
        ...
        akordeon: function (options) {
            var settings = $.extend({ expandedItem: 0, expandSpeed: 200, toggle: false, expandText: '&ndash;', collapseText: '+', buttons: true, hiddenItem: -1, itemsOrder: [], abertaPrimeira: false, corIcon: 'blue' }, options);
            return this.each(function () {
                ...
                var abertaPrimeira = settings.abertaPrimeira;
                var corIcon = settings.corIcon;
                ...
                $('.akordeon-item', akordeon).each(function () {
                    ...
                    if (settings.buttons)
                        $(this).find('.akordeon-item-head-container').prepend('<div class="akordeon-icon ' + corIcon + '"><span>' + collapseText + '</span></div>');
                });
