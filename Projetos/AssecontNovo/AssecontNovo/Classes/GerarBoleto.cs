...
namespace AssecontNovo.Classes
{
    public class GerarBoleto
    {
        ...
        public GerarBoleto(string CnpjCpf, DateTime DataPagamento, int BoId, BDAppAssefin bd)
        {
            this.bd = bd;
        ...
        public void getBol()
        {
            string s = "SELECT BOL_EMITIDOEM ,BOL_NOSSO_NUMERO ,BOL_NUMERO_DOCTO ,BOL_VALOR ," +
                    ...
                    ",CAD_RAZAOSOCIAL, CAD_ESTADO, CAD_CIDADE, CAD_ENDERECO ,CAD_ENDERECO_NUMERO ,CAD_BAIRRO, CAD_CEP ,BCO_NUMERO ,GRU_RAZAOSOCIAL ,CTAP_AGENCIA " +
                    ",CFGB_BOLETO_CARTEIRA ,CFGB_BOLETO_ACEITE ,CFGB_BOLETO_CODIGO_CLIENTE ,CFGB_BOLETO_LOCAL_PAGAMENTO " +
                    //",(CASE WHEN CAD_CNPJ IS NULL THEN CAD_CPF ELSE CAD_CNPJ END) AS CNPJ_CPF " +
            ...
            foreach (DataRow dr in table.Rows)
            {
                ...
                //this.ContaCedente = dr["BOLETO_CODIGO_BANCO"].ToString().Trim();
                this.ContaCedente = dr["BCO_NUMERO"].ToString().Trim();
                //+ dr["BOLETO_CC_DIGITO"].ToString().Trim();
                ...
                //this.CnpjCpf = dr["CNPJ_CPF"].ToString().Trim().Replace("-", "").Replace("/", "").Replace(".", "").PadLeft(14, '0');

                //this.digitoCedente = dr["BOLETO_CC_DIGITO"].ToString().Trim();
                this.CodigoCedente = dr["CFGB_BOLETO_CODIGO_CLIENTE"].ToString().Trim();
                ...
                //Double valor = Convert.ToDouble(dr["BOLETO_VALOR"].ToString());
        ...
        #region BoletoBancario Santander
        public BoletoBancario BoletoSantader()
        {
            ...
            Boleto Boleto = new Boleto(Vencimento, Convert.ToDecimal(ValorBoleto), NumeroCarteira, NossoNumeroBoleto.PadLeft(12, '0'), Cedente);
            ...
            Instrucao_Santander santander = new Instrucao_Santander();
            //Instrucao_Itau itau = new Instrucao_Itau();
            santander.Descricao = DescricaoBoleto;

            Boleto.Instrucoes.Add(santander);
            Boleto.EspecieDocumento = new EspecieDocumento_Santander(Especie);
            Boleto.NumeroDocumento = NumeroDocumento;
            //'Espécie do Documento - [R] Recibo;


            BoletoBancario boletobancario = new BoletoBancario();
            boletobancario.CodigoBanco = 33; //'-> Referente ao código do santander;
