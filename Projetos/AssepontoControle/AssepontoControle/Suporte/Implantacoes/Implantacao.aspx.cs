...
{
    public partial class Implantacao1 : System.Web.UI.Page
    {
        private BDApp bd;
        ...
        {
            string query;
            if (!string.IsNullOrEmpty(rblStatusAndamentoTipo.SelectedValue))
            {
                if (inicioImplantacao == "NULL")
                {
                    query = "UPDATE AssepontoImplantacoes SET IM_ANDAMENTO_TIPO = " + rblStatusAndamentoTipo.SelectedValue.Replace(",", "") + ", " +
                                "IM_ANDAMENTO = " + rblStatusAndamento.SelectedValue.Replace(",", "") + ", " + "IM_INICIO_IMPLANTACAO = GETDATE() " +
                                "WHERE IM_IND = " + indImplantacao;
                }
                else
                {
                    query = "UPDATE AssepontoImplantacoes SET IM_ANDAMENTO_TIPO = " + rblStatusAndamentoTipo.SelectedValue.Replace(",", "") + ", " +
                                "IM_ANDAMENTO = " + rblStatusAndamento.SelectedValue.Replace(",", "") +
                                "WHERE IM_IND = " + indImplantacao;
                }
            }
            else
            {
                if (inicioImplantacao == "NULL")
                {
                    query = "UPDATE AssepontoImplantacoes SET IM_ANDAMENTO = " + rblStatusAndamento.SelectedValue.Replace(",", "") + ", " + 
