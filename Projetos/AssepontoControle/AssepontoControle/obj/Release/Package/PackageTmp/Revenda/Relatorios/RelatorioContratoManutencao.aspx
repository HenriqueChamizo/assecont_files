<%@ Page Title="" Language="C#" MasterPageFile="~/MasterPages/Revenda.Master" AutoEventWireup="true" CodeBehind="RelatorioContratoManutencao.aspx.cs" Inherits="Asseponto.Revenda.Relatorios.RelatorioContratoManutencao" %>
...
<asp:Content ID="Content3" ContentPlaceHolderID="contentplaceConteudo" runat="server">
    <table style="padding-top: 20px; padding-bottom: 30px;">
        <tr>
            <td>
                ...
                <telerik:RadComboBox ID="ComboboxManutencao" Width="150" CausesValidation="false" runat="server" AutoPostBack="true"
                    OnSelectedIndexChanged="ComboboxManutencao_SelectedIndexChanged" Skin="flv" EnableEmbeddedSkins="false">
                    <Items>
                        <telerik:RadComboBoxItem Text="Selecione" Value="-1" />
            ...
            <td>
                ...
                <telerik:RadComboBox ID="ComboboxEnviado" Width="150" CausesValidation="false" runat="server" AutoPostBack="true"
            ...
            <td>
                ...
                <telerik:RadComboBox ID="ComboboxRecebido" Width="150" CausesValidation="false" runat="server" AutoPostBack="true"
            ...
            <td>
                ...
                <telerik:RadComboBox ID="ComboboxUF" Width="150" CausesValidation="false" runat="server" AutoPostBack="true"
    ...
    <asp:SqlDataSource ID="SqlDataContrato" runat="server" ConnectionString="<%$ ConnectionStrings:AppConnectionString %>" OnSelecting="SqlDataContrato_Selecting"
        SelectCommand="SELECT DISTINCT 
	                        (CASE WHEN MNT_REVENDA IS NULL 
		                          THEN 'Cliente' 
		                          ELSE 'Revenda' 
	                         END) AS REVCLI, 
	                        (CASE WHEN MNT_REVENDA IS NULL 
		                          THEN A.CAD_RAZAOSOCIAL 
		                          ELSE REV_RAZAOSOCIAL 
	                         END) AS RAZAOSOCIAL, 
	                        (CASE WHEN MNT_REVENDA IS NULL 
		                          THEN dbo.fn_cliente_cnpj_cpf(A.CAD_IND) 
		                          ELSE dbo.fn_revenda_cnpj_cpf(REV_IND) 
	                         END) AS CNPJ_CPF, 
                        ...
                        WHERE REV_IND = @REVENDA AND 
	                          (MNT_CANCELADO = @MANUTENCAO OR @MANUTENCAO = -1) AND 
	                          (UF = @UF OR @UF = '00' OR @UF IS NULL) AND
	                          ((@RECEBIDO = 1 AND MNT_CONTRATO_RECEBIDOEM IS NOT NULL) OR 
	                           (@RECEBIDO = 0 AND MNT_CONTRATO_RECEBIDOEM IS NULL) OR 
	                           @RECEBIDO = -1) AND
	                          ((@ENVIADO = 1 AND MNT_CONTRATO_ENVIADOEM IS NOT NULL) OR 
	                           (@ENVIADO = 0 AND MNT_CONTRATO_ENVIADOEM IS NULL) OR 
	                           @ENVIADO = -1)">
        <SelectParameters>
    ...
    <%--<asp:SqlDataSource ID="SqlDataUF" runat="server" ConnectionString="<%$ ConnectionStrings:AppConnectionString %>" OnSelecting="SqlDataUF_Selecting"
        SelectCommand="SELECT DISTINCT A.CAD_UF, (A.CAD_UF + ' (' + CONVERT(VARCHAR(10), COUNT_UF) + ') ') AS UF_COUNT
                        ...
                        (
	                        ...
	                        left JOIN AssepontoRevendas ON C.MNT_REVENDA = REV_IND
	                        WHERE B.CAD_UF = A.CAD_UF AND B.CAD_UF IS NOT NULL AND B.CAD_UF != '' AND
	                        MNT_CANCELADO = @MANUTENCAO AND REV_IND = @REVENDA AND 
